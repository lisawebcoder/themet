...
<h2 class="render-title"><?php _e('Favicon Image', 'sakela'); ?></h2>
<p><?php _e('The preferred size of the favicon is 16x16 pixels.', 'sakela'); ?></p>

<?php
    $logo_prefence = osc_get_preference('favicon', 'sakela');
?>
<?php if( is_writable( osc_uploads_path()) ) { ?>
    <?php if($logo_prefence) { ?>
        <h3 class="render-title"><?php _e('Preview', 'sakela') ?></h3>
        <img style="max-width:100%;" border="0" alt="<?php echo osc_esc_html( osc_page_title() ); ?>" src="<?php echo sakela_favicon_url().'?'.filemtime(osc_uploads_path() . osc_get_preference('favicon','sakela'));?>" />
