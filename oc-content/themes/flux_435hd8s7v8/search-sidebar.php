...
<div class="filters">
    <form action="<?php echo osc_base_url(true); ?>" method="get" class="nocsrf">
        ...
        <fieldset>
            <h3><?php _e('Show only', 'flux') ; ?></h3>
            <div class="row">
                <input type="checkbox" name="bPic" id="withPicture" value="1" <?php echo (osc_search_has_pic() ? 'checked' : ''); ?> />
                <label for="withPicture"><?php _e('listings with pictures', 'flux') ; ?></label>
            </div>
        </fieldset>
        <?php } ?>
        ...
        <fieldset>
            <div class="row price-slice">
                <h3><?php _e('Price', 'flux') ; ?></h3>
                <span><?php _e('Min', 'flux') ; ?>.</span>
        ...
        <?php } ?>
        <div class="actions">
            <button type="submit"><?php _e('Apply', 'flux') ; ?></button>
        </div>
    </form>
    <fieldset>
        <div class="row ">
            <h3><?php _e('Refine category', 'flux') ; ?></h3>
            <?php flux_sidebar_category_search($category['pk_i_id']); ?>
        </div>
    </fieldset>
</div>
</div>