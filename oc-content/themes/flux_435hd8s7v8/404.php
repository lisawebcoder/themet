...
<?php
    ...
    flux_add_body_class('error not-found');
    osc_current_web_theme_path('header.php') ;
?>
<div class="flashmessage-404">
    <h1><?php _e("Sorry but I can't find the page you're looking for", 'flux') ; ?></h1>

    <p><?php _e("Let us help you, we have got a few tips for you to find it.", 'flux') ; ?></p>
    <ul>
        <li>
            <?php _e("<strong>Search for it</strong>", 'flux') ; ?>
            <form action="<?php echo osc_base_url(true) ; ?>" method="get" class="search">
                <input type="hidden" name="page" value="search" />
                <fieldset class="main">
                    <input type="text" name="sPattern"  id="query" value="" />
                    <button type="submit" class="ui-button ui-button-middle"><?php _e('Search', 'flux') ; ?></button>
                </fieldset>
            </form>
        </li>
        <li><?php _e("<strong>Look for it in the most popular categories.</strong>", 'flux') ; ?>
