...
<html xmlns="http://www.w3.org/1999/xhtml" dir="ltr" lang="<?php echo str_replace('_', '-', osc_current_user_locale()); ?>">
    <head>
		...
		<?php
		if(osc_count_items() == 0 || Params::getParam('iPage') > 0 || stripos($_SERVER['REQUEST_URI'], 'search'))
		{
			?>
		...
		<style>
			...
			.chbx.checked{
				background-image:url('<?php echo osc_current_web_theme_url('images/checkmark.gif'); ?>');
			}
			.chbx.semi-checked{
            ...
            <div id="main">
                <div class="ad_list">
					...
					if(osc_count_items() == 0)
					{
						...
						{
							echo '<p class="empty" >' . sprintf(__('There are no results matching "%s"', 'minimalist'), osc_search_pattern()) . '</p>';
						}
						else
						{
							echo '<p class="empty" >' . __('There are no results', 'minimalist') . '</p>';
						}
					}
					else
					{

						// no gallery view for this theme
                ...
                <div class="filters">
                    <form action="<?php echo osc_base_url(true); ?>" method="get" onsubmit="return doSearch()" class="nocsrf">
                        ...
                        <fieldset class="box location">
                            <h3><strong><?php _e('Your search', 'minimalist'); ?></strong></h3>
                            ...
                            <div class="row one_input">
                                ...
                                <input type="hidden" id="sRegion" name="sRegion" value="<?php echo clb_minimalist_theme_search_region_id(); ?>" />
                            </div>
                        </fieldset>
                        ...
                        <fieldset class="box show_only">
								...
								<div class="row checkboxes">
									<ul>
										<li>
											...
											<label for="withPicture"><?php _e('Show only listings with pictures', 'minimalist'); ?></label>
										</li>
									</ul>
								</div>
								<?php
							...
							if(osc_price_enabled_at_items())
							{
								...
								<div class="row two_input">
									<h6><?php _e('Price', 'minimalist'); ?></h6>
									<div class="c1">
										<label for="priceMin"><?php _e('Min', 'minimalist'); ?>.</label>
										<input type="text" id="priceMin" name="sPriceMin" value="<?php echo osc_esc_html(osc_search_price_min()); ?>" size="6" maxlength="6" />

									</div>
									<div class="c2">
										<label for="priceMax"><?php _e('Max', 'minimalist'); ?>.</label>
							...
							if(osc_count_categories())
							{
								?>
								<div class="row checkboxes">
									<h6><?php _e('Category', 'minimalist'); ?></h6>
									<ul>
										<?php
										...
										while(osc_has_categories())
										{
											?>
											<li class="parent">
												<input class="parent" type="checkbox" id="cat<?php echo osc_category_id(); ?>" name="sCategory[]" value="<?php echo osc_category_id(); ?>" <?php
												$parentSelected = false;
												if(in_array(osc_category_id(), osc_search_category()) || in_array(osc_category_slug() . "/", osc_search_category()) || in_array(osc_category_slug(), osc_search_category()) || count(osc_search_category()) == 0)
												{
												...
												if(osc_count_subcategories() > 0)
												{
													...
													<ul class="sub">
														...
														while(osc_has_subcategories())
														{
															?>
															<li>
																<input type="checkbox" id="cat<?php echo osc_category_id(); ?>" name="sCategory[]" value="<?php echo osc_category_id(); ?>"  <?php
						...
						?>
						<div class="row">
							<button type="submit"><?php _e('Apply', 'minimalist'); ?></button>
						</div>
                    </form>
					<?php osc_alert_form(); ?>
                </div>
				...
				$(function() {
					...
					$("#sCity").autocomplete({
						...
						select: function(event, ui) {
							...
							log(ui.item ?
									"<?php _e('Selected', 'minimalist'); ?>: " + ui.item.value + " aka " + ui.item.id :
									"<?php _e('Nothing selected, input was', 'minimalist'); ?> " + this.value);
						}
					});
				});
